...
function ts_theme_styles() 
{
    ...
    if($enable_inline_css < 1) :
        ...
        //wp_enqueue_style( 'theme-options-css', get_template_directory_uri().'/css/style.css.php', '', TS_THEME_VERSION.$ts_php_css_ver);
        wp_enqueue_style( 'theme-options-css', get_bloginfo('url').'?theme-options=css', '', TS_THEME_VERSION.$ts_php_css_ver);
    endif;
    
    /*
    if(ts_option_vs_default('override_ubermenu_styling', 1) == 1 && function_exists('uberMenu_direct')) :
        wp_enqueue_style( 'override-basic-ubermenu', get_template_directory_uri().'/css/ubermenu.css', '', TS_THEME_VERSION.$ts_php_css_ver);
    endif;
    */
...
function ts_theme_scripts() 
{
    ...
    wp_enqueue_script( 'googlemaps', '//maps.googleapis.com/maps/api/js?sensor=false', array('jquery'), '', true);
    wp_enqueue_script( 'owl-carousel', get_template_directory_uri().'/js/owl.carousel.min.js', array('jquery'), '', true);
	wp_enqueue_script( 'ts-init', get_template_directory_uri().'/js/init.js', array('jquery','ts-plugins'), TS_THEME_VERSION, true);
...
{        
    ...
    function ts_enqueue_color_picker( ) {
        ...
        wp_enqueue_script( 'ts-admin-scripts', get_template_directory_uri().'/js/admin-scripts.js', array( 'wp-color-picker' ), false, true );
    }

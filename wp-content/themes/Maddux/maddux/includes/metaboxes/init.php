...
class cmb_Meta_Box {
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( !isset( $field['unhide_id'] ) ) $field['unhide_id'] = '';
			if ( !isset( $field['unhide_value'] ) ) $field['unhide_value'] = '';
            ...
            if(trim($field['unhide_id']) && ($field['unhide_value'])) {
                $field['unhide_value'] = (is_array($field['unhide_value'])) ? $field['unhide_value'] : array($field['unhide_value']);
                $unhide_value = json_encode($field['unhide_value']); 
            ?>
                <script type="text/javascript">
                jQuery(function($) {
                    
                    if(jQuery.inArray( jQuery('#<?php echo esc_attr($field['unhide_id']);?>').val(), <?php echo $unhide_value;?> ) >= 0) {
                        jQuery('#cmb_field_row_<?php echo esc_attr($field['id']);?>').show();
                    } else {
                        jQuery('#cmb_field_row_<?php echo esc_attr($field['id']);?>').hide();
                    }
                    jQuery('#<?php echo esc_attr($field['unhide_id']);?>').on('change', function() {
                        if(jQuery.inArray( jQuery('#<?php echo esc_attr($field['unhide_id']);?>').val(), <?php echo $unhide_value;?> ) >= 0) {
                            jQuery('#cmb_field_row_<?php echo esc_attr($field['id']);?>').show();
                        } else {
                            jQuery('#cmb_field_row_<?php echo esc_attr($field['id']);?>').hide();
            ...
            $cmb_hidden_style = ($field['hidden'] === true) ? 'style="display:none"' : '';
			echo '<tr id="cmb_field_row_'.esc_attr($field['id']).'" '.$cmb_hidden_style.'>';

			if ( $field['type'] == "title" ) {
				echo '<td colspan="2">';
			} else {
				if( $this->_meta_box['show_names'] == true ) {
					echo '<th style="width:18%"><label for="', esc_attr($field['id']), '">', $field['name'], '</label></th>';
				}
				echo '<td>';
			}
			...
			switch ( $field['type'] ) {
				...
				case 'text':
					echo '<input type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" />','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'text_small':
					echo '<input class="cmb_text_small" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'text_medium':
					echo '<input class="cmb_text_medium" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'text_date':
					echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'text_date_timestamp':
					echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr(date( 'm\/d\/Y', $meta )) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;

				case 'text_datetime_timestamp':
					echo '<input class="cmb_text_small cmb_datepicker" type="text" name="', esc_attr($field['id']), '[date]" id="', esc_attr($field['id']), '_date" value="', '' !== $meta ? esc_attr(date( 'm\/d\/Y', $meta )) : esc_attr($field['std']), '" />';
					echo '<input class="cmb_timepicker text_time" type="text" name="', esc_attr($field['id']), '[time]" id="', esc_attr($field['id']), '_time" value="', '' !== $meta ? esc_attr(date( 'h:i A', $meta )) : esc_attr($field['std']), '" /><span class="cmb_metabox_description" >', $field['desc'], '</span>';
					break;
				case 'text_time':
					echo '<input class="cmb_timepicker text_time" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'text_money':
					echo '$ <input class="cmb_text_money" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'colorpicker':
					...
					elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
						$meta = "#";
					echo '<input class="cmb_colorpicker cmb_text_small" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', esc_attr($meta), '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'textarea':
					echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="10">', '' !== $meta ? esc_textarea($meta) : esc_textarea($field['std']), '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'textarea_small':
					echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="4">', '' !== $meta ? esc_textarea($meta) : esc_textarea($field['std']), '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'textarea_code':
					echo '<textarea name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" cols="60" rows="8" class="cmb_textarea_code">', '' !== $meta ? esc_textarea($meta) : esc_textarea($field['std']), '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'select':
					...
					foreach ($field['options'] as $option) {
						...
						echo '<option value="'.esc_attr($option['value']).'" '.$selected.'>'.$option['name'].'</option>';
					}
					echo '</select>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'select_slider_cats':
				
                    $ts_slider_categories = ts_get_categories('slider-category', 0, 'cb_metabox_array');
                    
                    /*
                    array_unshift(
                        $ts_slider_categories
                    );
                    */
                    
					$default = (isset($meta)) ? $meta : $field['std'];
					echo '<select name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '">';
					foreach ($ts_slider_categories as $option) {
                        $selected = ($default == $option['value']) ? 'selected="selected"' : '';
						echo '<option value="'.esc_attr($option['value']).'" '.$selected.'>'.$option['name'].'</option>';
					}
					echo '</select>';
				...
				case 'radio_inline':
					...
					$i = 1;
					foreach ($field['options'] as $option) {
						echo '<div class="cmb_radio_inline_option"><input type="radio" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), $i, '" value="', esc_attr($option['value']), '"', $meta == $option['value'] ? ' checked="checked"' : '', ' /><label for="', esc_attr($field['id']), $i, '">', $option['name'], '</label></div>';
						$i++;
					}
					echo '</div>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
				...
				case 'radio':
					...
					$i = 1;
					foreach ($field['options'] as $option) {
						echo '<li><input type="radio" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), $i,'" value="', esc_attr($option['value']), '"', $meta == $option['value'] ? ' checked="checked"' : '', ' /><label for="', esc_attr($field['id']), $i, '">', $option['name'].'</label></li>';
						$i++;
					}
					echo '</ul>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
				...
				case 'multicheck':
					...
					foreach ( $field['options'] as $value => $name ) {
						...
						echo '<li><input type="checkbox" name="', esc_attr($field['id']), '[]" id="', esc_attr($field['id']), $i, '" value="', esc_attr($value), '"', in_array( $value, $meta ) ? ' checked="checked"' : '', ' /><label for="', esc_attr($field['id']), $i, '">', $name, '</label></li>';
						$i++;
					}
					echo '</ul>';
					echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'title':
					echo (trim($field['name'])) ? '<h5 class="cmb_metabox_title">'. $field['name']. '</h5>' : '';
					echo (trim($field['desc'])) ? '<p class="cmb_metabox_description">'. $field['desc']. '</p>' : '';
					break;
				case 'wysiwyg':
				...
				case 'taxonomy_select':
					...
					foreach ( $terms as $term ) {
						if (!is_wp_error( $names ) && !empty( $names ) && !strcmp( $term->slug, $names[0]->slug ) ) {
							...
							echo '<option value="' . esc_attr($term->slug) . '  ' , $meta == $term->slug ? esc_attr($meta) : ' ' ,'  ">' . $term->name . '</option>';
						}
					}
					echo '</select>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
				...
				case 'taxonomy_radio':
					...
					foreach ( $terms as $term ) {
						if ( !is_wp_error( $names ) && !empty( $names ) && !strcmp( $term->slug, $names[0]->slug ) ) {
							echo '<li><input type="radio" name="', esc_attr($field['id']), '" value="'. esc_attr($term->slug) . '" checked>' . $term->name . '</li>';
						} else {
							echo '<li><input type="radio" name="', esc_attr($field['id']), '" value="' . esc_attr($term->slug) . '  ' , $meta == $term->slug ? $meta : ' ' ,'  ">' . $term->name .'</li>';
						}
					}
					echo '</ul>';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
				...
				case 'taxonomy_multicheck':
					...
					$terms = get_terms( $field['taxonomy'], 'hide_empty=0' );
					foreach ($terms as $term) {
						echo '<li><input type="checkbox" name="', esc_attr($field['id']), '[]" id="', esc_attr($field['id']), '" value="', esc_attr($term->term_id) , '"';
					...
					echo '<div id="', esc_attr($field['id']), '_status" class="cmb_media_status">';
						if ( $meta != '' ) {
							...
							if ( $check_image ) {
								...
								echo 'File: <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', esc_url($meta), '" target="_blank" rel="external">Download</a> / <a href="#" class="cmb_remove_file_button" rel="', esc_attr($field['id']), '">Remove</a>)';
							}
						}
					echo '</div>';
				break;
				case 'oembed':
					echo '<input class="cmb_oembed" type="text" name="', esc_attr($field['id']), '" id="', esc_attr($field['id']), '" value="', '' !== $meta ? esc_attr($meta) : esc_attr($field['std']), '" />','<p class="cmb_metabox_description">', $field['desc'], '</p>';
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
            ...
            $field['allow_html'] = (isset($field['allow_html'])) ? $field['allow_html'] : '';
			...
			if ( ($field['type'] == 'textarea_code') || ($field['allow_html'] == true)) {
...
function cmb_oembed_ajax_results() {
	...
	if ( empty( $oembed_string ) ) {
		...
		if ( $check_embed && $check_embed != $fallback ) {
			// Embed data
			$return = '<div class="embed_status">'. $check_embed .'<a href="#" class="cmb_remove_file_button" rel="'. esc_attr($_REQUEST['field_id']) .'">'. __( 'Remove Embed', 'cmb' ) .'</a></div>';
			// set our response id
			$found = 'found';
